...
namespace SPGen2008
{
	public partial class CStoredProcedure : UserControl
	{
		...
		private void CStoredProcedure_Load(object sender, EventArgs e)
		{
			...
			{
				_ResultType_comboBox.Items.Add(t);
			}
			foreach (Microsoft.SqlServer.Management.Smo.View v in uvs)
			{
				_ResultType_comboBox.Items.Add(v);
			}
			foreach(UserDefinedTableType ut in udtts)
			{
				_ResultType_comboBox.Items.Add(ut);
			}
			...
			else _ResultType_comboBox.SelectedIndex = _ResultType_comboBox.FindString(this.ResultType);
