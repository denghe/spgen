...
ORDER BY b.numof DESC");
        ...
        /// </summary>
        public static DbSet ExecuteDbSet(string s)
        {
            DbSet ds = new DbSet();
            SqlConnection conn = CurrConn;
            int ocs = -1;
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    do
                    {
                        ...
                        if (r.HasRows)
                            while (r.Read())
                            {
                                object[] buffer = new object[t.Columns.Count];
                                r.GetValues(buffer);
                                t.NewRow(buffer);
                            }
                    } while (r.NextResult());
        ...
        /// </summary>
        public static DbTable ExecuteDbTable(string s)
        {
            DbTable t = new DbTable();
            SqlConnection conn = CurrConn;
            int ocs = -1;
        ...
        /// </summary>
        public static DbSet ExecuteDbSet(SqlCommand cmd)
        {
            DbSet ds = new DbSet();
            SqlConnection conn = null;
            if (cmd.Connection == null) conn = CurrConn;
        ...
        /// 执行一个 SQL 命令对象，返回一个 DbSet 数据集
        /// </summary>
        public static DbTable ExecuteDbTable(SqlCommand cmd)
        {
            DbTable t = new DbTable();
            SqlConnection conn = null;
            if (cmd.Connection == null) conn = CurrConn;
